...
class InsuranceModelInsuranceform extends JModelAdmin{
	
	
	public function getTable($type = 'Insuranceform', $prefix = 'InsuranceTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_insurance.insuranceform', 'insuranceform', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_insurance.edit.insuranceform.data', array());
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (empty($table->ordering)) {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__insurance');
				$max = $db->loadResult();
